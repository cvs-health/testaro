...
exports.markOperable = async page => {
  ...
  // Recursively finds and marks the elements that manage descendants’ pseudofocus.
  const managerOperable = async (page, elements) => {
    // If any elements remain unprocessed:
    if (elements.length) {
      // Identify the first of them.
      const firstElement = elements[0];
      // Determine whether it needs to be marked because of an aria-activedescendant property.
      const isMarkable = await page.evaluate(
        element =>
          ! element.dataset.autotestOperable
      ...
      if (isMarkable) {
        // Mark it as operable.
        await mark(page, firstElement, 'manager');
      }
      // Process the remaining elements.
      await managerOperable(page, elements.slice(1));
    }
  };
  ...
  // Recursively mark elements with aria-activedescendant properties as operable.
  await managerOperable(page, elements);
};
